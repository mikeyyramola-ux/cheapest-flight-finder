...
export async function handleStripeWebhook(payload: Buffer, signature?: string) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    console.info("[Stripe] Premium checkout completed", { eventId: event.id, userId: session.metadata?.user_id, customerId: session.customer, subscriptionId: session.subscription });
    // Persist customer/subscription identifiers on the user row in a production database.
  }
