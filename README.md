...
The Book now buttons fall back to a fixed Travelpayouts/Kiwi link until you configure your own. Generate a flight-partner deep link in Travelpayouts and add it in Vercel as the public variable `VITE_TRAVELPAYOUTS_AFFILIATE_URL`. The following literal placeholders are replaced per result at click time: `{subid}` (route-specific marker such as `faredrop-jfk-lhr-sample-1`), `{origin}` and `{destination}` (IATA codes), `{depart}` and `{return}` (DDMM date digits for Aviasales-style search URLs, empty for one-way trips), and `{passengers}`. Unknown placeholders are left unchanged, so a plain fixed link works exactly as before. Stays and Cars searches can be monetized the same way with `VITE_BOOKING_AFFILIATE_URL` / `VITE_RENTALCARS_AFFILIATE_URL` using the placeholders `{location}`, `{start}`, `{end}`, `{guests}`; when unset, the plain partner search URLs are used. An official Travelpayouts widget can be embedded verbatim via `VITE_TRAVELPAYOUTS_WIDGET_SNIPPET` (paste the exact snippet generated in the dashboard — partner widget code must never be modified). The full program-by-program launch plan lives in `docs-monetization.md`. Do not label or advertise a link as affiliate-enabled until the partner URL is approved and tested.
